...
LIMIT 10;
-- Q4 (revised): Top 10 countries by energy consumption in 2022
-- Uses a stricter filter to exclude regional aggregates
SELECT country,
       ROUND(CAST(primary_energy_consumption AS REAL), 1) AS consumption_twh
FROM energy
WHERE CAST(year AS INTEGER) = 2022
  AND primary_energy_consumption != ''
  AND country NOT LIKE '%(EIA)%'
...
SELECT MIN(CAST(year AS INTEGER)) AS earliest_year_with_data
FROM energy
WHERE country = 'World'
  AND renewables_share_energy != '';